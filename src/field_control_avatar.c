...
static bool32 CheckPartyForMove(enum Move move) {
    u8 i;
...
static const u8 *GetInteractedWaterScript(struct MapPosition *unused1, u8 metatileBehavior, enum Direction direction)
{
    ...
    if ((IsFieldMoveUnlocked(FIELD_MOVE_SURF) == TRUE || PartyHasMonWithSurf() == TRUE || CheckPartyForMove(MOVE_SURF) == TRUE) && IsPlayerFacingSurfableFishableWater() == TRUE
