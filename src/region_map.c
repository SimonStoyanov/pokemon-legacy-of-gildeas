...
{
    [MAPSEC_ARCHEVALE_TOWN] = {MAP_GROUP(MAP_ARCHEVALE_TOWN), MAP_NUM(MAP_ARCHEVALE_TOWN), HEAL_LOCATION_ARCHEVALE_TOWN_PLAYER_HOUSE_2F},
...
static u8 ProcessRegionMapInput_Zoomed(void)
{
    ...
    if (JOY_HELD(DPAD_UP) && sRegionMap->scrollY > -0x34 -8)
    {
        sRegionMap->zoomedCursorDeltaY = -1;
        input = MAP_INPUT_MOVE_START;
    }
    if (JOY_HELD(DPAD_DOWN) && sRegionMap->scrollY < 0x3c +16)
    {
        sRegionMap->zoomedCursorDeltaY = +1;
        input = MAP_INPUT_MOVE_START;
    }
    ...
    if (JOY_HELD(DPAD_RIGHT) && sRegionMap->scrollX < 0xac + 16)
    {
        sRegionMap->zoomedCursorDeltaX = +1;
        input = MAP_INPUT_MOVE_START;
    }
...
static u8 GetMapsecType(mapsec_u16_t mapSecId)
{
    ...
    case MAPSEC_MAROLIVA_TOWN:
        return FlagGet(FLAG_VISITED_MAROLIVA_TOWN) ?  MAPSECTYPE_CITY_CANFLY : MAPSECTYPE_CITY_CANTFLY;
    case MAPSEC_ARCHEVALE_TOWN:
        return FlagGet(FLAG_VISITED_ARCHEVALE_TOWN) ?  MAPSECTYPE_CITY_CANFLY : MAPSECTYPE_CITY_CANTFLY;
